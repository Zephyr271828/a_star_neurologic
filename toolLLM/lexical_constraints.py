...
Phrase = List[int] # int for target word id
Literal = Tuple[Phrase, bool] 
# whether a family of phrase is satisfied or not
...
# a list of sequences, each sequence corresponds to a list of literals
...
def init_batch(raw_constraints: List[ClauseConstraintList],
    ...
    for i, (raw_list, key_list) in enumerate(zip(raw_constraints, key_constraints)):
        ...
        # ConstrainedHypothesis is a class defined previously
        idx = i * beam_size
        constraints_list[idx:idx + beam_size] = [copy.deepcopy(hyp) for _ in range(beam_size)]
        # each row is hyp, with len(constraints) * beam_size rows in total    

...
def _generate_greedy(
    ...
    for t in range(look_ahead_step):
        ...
        #     past = outputs[1]
        if hasattr(outputs, "past_key_values") and self.config.use_cache:
        ...
        for j, (phrase, phrase_mask) in enumerate(phrases_idx_mask):
            if torch.sum(phrase_mask[:, t]).item():
                ...
                if len(phrase) > 1:
                    ...
                    phrase_position_ids = phrase_position_ids.view(-1, phrase_input_ids.shape[0]).T
                    # phrase_position_ids = None
                    ...
                    follow_logits = self(input_ids=phrase_input_ids, past_key_values=past, # edited
